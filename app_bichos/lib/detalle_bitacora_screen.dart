...
class DetalleBitacoraScreen extends StatefulWidget {
  ...
  final String nombreBitacora;
  

  const DetalleBitacoraScreen({super.key, required this.idBitacora, required this.nombreBitacora});
...
class _DetalleBitacoraScreenState extends State<DetalleBitacoraScreen> {
  ...
  Future<void> _cargarReportes() async {
    try {
      ...
      final response = await http.get(
        Uri.parse('$_baseUrl/reportes/bitacora/${widget.idBitacora}'),
  ...
  } // <-- Llave de cierre corregida. Aquí termina _cargarReportes.

  // Función extraída para que la interfaz pueda acceder a ella correctamente
  Future<void> _eliminarReporteBitacora(String idReporte) async {
                        ...
                        onPressed: procesando ? null : () async {
                          ...
                          try {
                            ...
                            if (response.statusCode == 200) {
                              ...
                              if (mounted) setState(() { _cargando = true; });
                              _cargarReportes(); // <-- Ajusta el nombre si estás en detalle_bitacora
                            }
                          } catch (e) {
                            print(e);
                          } finally {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(widget.nombreBitacora)),
      body: _cargando 
        ? const Center(child: CircularProgressIndicator())
        : _reportes.isEmpty 
            ? const Center(child: Text("Esta bitácora está vacía."))
            : ListView.builder(
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            IconButton(
                              ...
                              onPressed: () => _eliminarReporteBitacora(reporte['id_reporte']), // O _eliminarReporteBitacora
                            ),
                          ],
                        ),
                      ],
